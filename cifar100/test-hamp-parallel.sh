...
for alpha in "${alphas[@]}"; do
  ...
  for entropy_percentile in "${entropy_percentiles[@]}"; do
    # Loop over each tag
      ...
      srun -c 2 -G 1 -p gpu-preempt --nodes 1 --nodelist=gpu[013-041] --mem=40000 python ./model_training/train-hamp.py --scan_para 1 --entropy_percentile $entropy_percentile --alpha $alpha --config ./configs/$model/hamp.yml --isTrain 0 &
