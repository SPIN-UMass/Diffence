...
private_data_len= int(config.attack.train_size * 1.0) 

attack_tr_len=private_data_len 
attack_te_len=0  
tr_frac=0.5 
te_frac=0.5 
output_save_path=config.attack.output_folder
...
if not args.diff == 0:
    data_path = f'./parallel_run/data/{config.attack.target_model}/num_sample{config.attack.num_sample}/{config.attack.save_tag}_{config.attack.train_size}/diff{args.diff}/iter{config.purification.max_iter}_path{config.purification.path_number}_step{config.purification.purify_step}'
else:
    data_path = f'./parallel_run/data/{config.attack.target_model}/num_sample{config.attack.num_sample}/{config.attack.save_tag}_{config.attack.train_size}/diff{args.diff}'
...
print('tr len %d | mia_members tr %d te %d | mia_nonmembers tr %d te %d | ref len %d | test len %d'%
      (len(private_data_tensor),len(mia_train_members_data_tensor),len(mia_test_members_data_tensor),
       len(mia_train_nonmembers_data_tensor),len(mia_test_nonmembers_data_tensor),
       len(ref_data_tensor),len(te_data_tensor)), flush=True)
...
if args.diff == 1:
    ...
    best_model =  ModelwDiff_v3(best_model, config_path=args.config)
...
if(config.attack.prepMemGuard):
    ...
    defense_model_path = f'./parallel_run/memguard/'
...
# for test target model 
    ...
    print(len(private_data_tensor))
    print(time_cost/len(private_data_tensor))
    assert False
