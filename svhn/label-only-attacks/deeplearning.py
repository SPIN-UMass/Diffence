...
def train_shadow_model(args, targetmodel, shadowmodel, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        if batch_idx % len(train_loader)-1 == 0:
            print('ShadowModel Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch,
                batch_idx * len(data),
                len(train_loader.dataset),
                100. * batch_idx / len(train_loader),
...
def test_shadow_model(args, targetmodel, shadowmodel, test_loader, epoch, save=True):
    ...
    with torch.no_grad():
        for batch_idx, (data, label) in enumerate(test_loader):
            ...
            test_loss += F.cross_entropy(output, target).item()
            pred = output.max(1, keepdim=True)[1]
            # correct += pred.eq(target.view_as(pred)).sum().item() # using target model's label
            correct += pred.eq(label.view_as(pred)).sum().item() # use ground truth label
