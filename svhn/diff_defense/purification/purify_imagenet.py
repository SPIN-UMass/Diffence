...
def purify_imagenet(x, diffusion,model,  max_iter, mode, config):
    ...
    purify_step=config.purification.purify_step+1 #step list starts from 0
    images_list = []
    ...
    origin_size =x_adv.size()[-2:]
    resize_transform = transforms.Resize((256, 256))
    x_adv = resize_transform(x_adv)

    t_steps = torch.ones(x_adv.shape[0], device=config.device.diff_device).long()
    t_steps = t_steps * (purify_step-1)
    ...
    def cond_fn(x_reverse_t, t):
        ...
        with torch.enable_grad():
            ...
            x_adv_t = x_adv
            # scale = exp(config.purification.guide_exp_a * t / purify_step+config.purification.guide_exp_b) + config.purification.guide_scale_base
    ...
    with torch.no_grad():
        ...
        for i in range(max_iter):            
            ...
            # x_pur = torch.nn.functional.interpolate(x_pur, size=[218, 178], mode="bilinear") # transfrom size 256 -> 224
            reverse_resize_transform = transforms.Resize(origin_size)
            x_pur = reverse_resize_transform(x_pur)
